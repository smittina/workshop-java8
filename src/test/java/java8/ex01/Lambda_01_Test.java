...
public class Lambda_01_Test {
    ...
    private List<Person> filter(List<Person> persons, PersonPredicate predicate) {
       List<Person> laListe = new ArrayList<>();
    ...
    public void test_filter_by_password() throws Exception {
        ...
        List<Person> result = filter(personList, p-> p.getAge()>49 && DigestUtils.sha512Hex(p.getPassword()).equalsIgnoreCase(passwordSha512Hex) );
